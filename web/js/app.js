...
import { Rewards, MELODIES, SKINS, THEMES, SHOP, computeReport } from "./reward.js";
...
const fairyAtlas = new Image();
fairyAtlas.src = "assets/fairy/cheokcheok-atlas.png";
...
async function start() {
  ...
  // 백그라운드 탭에서는 rAF가 멈추므로 인터벌이 1Hz 감시를 이어받는다
  ...
  if (wallMs - lastDetect >= minGap && els.cam.readyState >= 2) {
    ...
    if (state === "BAD") {
      ...
      if (step >= 1 && !escFired) {
        ...
        notify(m2); playAlert(2, m2);
      } else if (step >= 1 && escFired && dur % TUNING.ESCALATE_NOTIFY < 1.2) {
        escFired = false; // 60초마다 재무장
      }
    }
    // 포인트 적립 (GOOD 1분당 +1P)
    if (rewards.tick(state, now, dateStr()) > 0) { flashPoints(); rewardUntil = now + 2.5; }
...
// 헤더 요정: 기본 스킨(척추요정)은 스프라이트 GIF, 구매 스킨(고양이·곰)은 이모지
...
function drawMini(state, score, face, badCand, now) {
  ...
  const useSprite = rewards.shop.skin === "fairy" && fairyAtlas.complete && fairyAtlas.naturalWidth > 0;
  if (useSprite) {
    ...
    ctx.drawImage(fairyAtlas, frame * SPRITE.cell, row * SPRITE.cell, SPRITE.cell, SPRITE.cell,
                  8, (h - 72) / 2, 72, 72);
    ctx.globalAlpha = 1;
  } else {
    ctx.font = "40px serif";
...
function openReport() {
  ...
  $("report-fairy").src = `assets/fairy/${gradeAnim}.gif`;
...
function renderShop() {
  ...
  for (const item of SHOP) {
    ...
    div.innerHTML = `<span>${item.label}</span><span class="price">${owned ? "보유 중" : item.price + "P"}</span>`;
    if (!owned) {
...
async function uploadStats() {
  const g = myGroup();
  if (!g) return;
  try { await api("stats", { memberId, week: weekKey(), goodSec: weeklyGoodSec(), points: rewards.points }); } catch {}
}
async function refreshLeaderboard() {
  ...
  try {
    ...
    board.innerHTML = "<table><tr><th>#</th><th>닉네임</th><th>바른 자세</th><th>포인트</th></tr>" +
      rows.map((r, i) =>
        `<tr class="${r.member_id === memberId ? "me" : ""}"><td>${i + 1}</td><td>${r.nickname}</td>` +
        `<td>${(r.good_sec / 60).toFixed(0)}분</td><td>${r.points}P</td></tr>`).join("") +
      "</table>";
  } catch (e) {
    board.innerHTML = "";
